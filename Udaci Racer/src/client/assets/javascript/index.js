...
function setupClickHandlers() {
	document.addEventListener('click', function(event) {
		...
		if(ele.matches('.card.track')){
			handleSelectTrack(ele);
		}

		if(ele.matches('.card.podracer')){
			handleSelectPodRacer(ele);
		}
...
async function runRace(raceID) {
	...
	*/
		const raceInterval = setInterval(async ()=> {
			...
			.catch((error) => console.log("Problem with getRace error ", error) 
			);
...
function raceProgress(positions) {
	...
	const results = positions.map(p => {
		...
		if(p.id == store.player_id) {
			...
			<tr>
				<td>
					<h3 id="Bottom">${count++} - <span id="player">${RacerName[p.driver_name]}</span></h3>
				</td>
			</tr>
		`
		}
			...
			return `
				<tr>
					<td>
						<h3 id="Bottom">${count++} - ${RacerName[p.driver_name]}</h3>
					</td>
				</tr>
			`
		}
...
async function getTracks() {
	...
	try {
		...
		console.log("Problem with getTracks: ", err);
	}
	}
}
...
async function getRacers() {
	...
	try {
		...
		console.log("Problem with getRacers :: ", err);
	}
	}
}
